...
@time for j in (eachindex(it_vec))
    for k in ProgressBar(1:epochs)
        for i in (1:2)
            ...
            params = SinkhornParameters(ε=ε,q=q,Δ=Δ,s=s,crit_it=crit_it,p_ω=p_ω,tol=tol,max_it=it_vec[j],sym=false,acc=acc)
...
V = SinkhornVariable(X,α)
W = SinkhornVariable(Y,β)
params = SinkhornParameters(ε=ε,q=q,Δ=Δ,s=s,crit_it=crit_it,p_ω=p_ω,tol=tol,max_it=it_vec[end],sym=false,acc=true)
S = SinkhornDivergence(V,W,CC,params,true)
initialize_potentials!(S)
compute!(S)
