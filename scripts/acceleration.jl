...
ε = 0.1 * N^(-1/(d′+4))
s = ε
crit_it = 16 # Int(ceil(0.2 * Δ / ε)) 
p_ω = 2


# for max_it = 10, 20, ... calculate the distance between two uniform distributions.
# maximum iterations are 10*i_max
i_max = 16 #Int(ceil(Δ/ε/10))
it_vec = [ 4*i for i in 1:i_max ]
...
@time for j in (eachindex(it_vec))
    for k in ProgressBar(1:epochs)
        for i in (1:2)
            if i == 1
                acc = false
            else
            ...
            params = SinkhornParameters(CC;ε=ε,q=q,Δ=Δ,s=s,crit_it=crit_it,p_ω=p_ω,tol=tol,max_it=it_vec[j],sym=false,acc=acc)
...
V = SinkhornVariable(X,α)
W = SinkhornVariable(Y,β)
params = SinkhornParameters(CC;ε=ε,q=q,Δ=Δ,s=s,crit_it=crit_it,p_ω=p_ω,tol=tol,max_it=it_vec[end],sym=false,acc=true)
S = SinkhornDivergence(V,W,CC,params)
initialize_potentials!(V,W,CC)
compute!(S)
...
p = plot(it_vec, mean(abs.(marginal_error_vec[1,:,:]), dims = 2), left_margin = 4mm, ylim=(1e-16,1e-1),
        minorgrid = true, xlabel = L"$\mathrm{iteration}$", ylabel = L"$\int \vert 1 - \int \exp ( (\phi \oplus \psi - c) / \varepsilon ) d n^\beta \vert d n^\alpha$", yaxis=:log,
