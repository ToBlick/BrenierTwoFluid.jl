...
### parameters
d = 2
c = (x,y) -> c_periodic(x,y,[1.0, 1.0]) # 0.5 * sqeuclidean(x,y)
∇c = (x,y) -> ∇c_periodic(x,y,[1.0, 1.0]) #x-y

d′ = 2*floor(d/2)
δ = 0.05    # spatial tolerance 
ε = 10 * δ^2     # entropic regularization parameter

N = 32^2 #Int((ceil(1e-1/ε))^(d))  
